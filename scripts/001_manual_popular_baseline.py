...
def count_popular_items(path, event_types, max_sessions=None):
    event_types = set (event_types) #集合去重
    counts = Counter()
...
#验证
def evaluate_leave_last(train_path, labels_by_type, max_sessions=None):
    ...
    for session in iter_sessions(train_path, max_sessions=max_sessions): 
        ...
        if target_type not in TASK_TYPE:
            continue
...
def write_submission(test_path, output_path, labels_by_tye, max_sessions=None):
    ...
    with output_path.open("w", newline="", encoding="utf-8") as f:
        ...
        for session in iter_sessions(test_path, max_sessions=max_sessions):
            ...
            for task_type in TASK_TYPE:
                labels = " ".join(str(aid) for aid in labels_by_tye[task_type])
...
def main():
    ...
    parser.add_argument("--output", default="projects/otto-session-recsys/scripts/manual_popular_submission.csv")
    parser.add_argument("--top-k", type=int, default=20)
    parser.add_argument("--max-sessions", type=int, default=None)
    parser.add_argument(
        "--popular-by",
        default="orders",
        help="Use commad-seaparated event types, e.g. orders or clicks,carts. Use per_type for one top lsit per task."
    ...
    if popular_by is None:
        ...
        labels_by_type = {task_type: labels for task_type in TASK_TYPE}
        print(f"gobal top-{args.top_k} by {popular_by}: {labels}")
